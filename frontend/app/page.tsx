...
export default function CinematicListingApp() {
  ...
  const handleForceDownload = async () => {
    ...
    try {
      ...
      a.href = blobUrl;
      const safeName = meta.address ? meta.address.replace(/[^a-z0-9]/gi, '_').toLowerCase() : 'cinematic-listing';
  ...
  const removeScene = (index: number) => setScenes(scenes.filter((_, i) => i !== index));

  const StepIndicator = () => (
    <div className="flex items-center justify-center gap-2 mb-8 text-sm font-medium">
      <div className={`flex items-center gap-2 ${step >= 1 ? 'text-indigo-400' : 'text-neutral-600'}`}>
        <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${step >= 1 ? 'bg-indigo-500/20' : 'bg-neutral-800'}`}>1</span> Extract
      </div>
      <ChevronRight className="w-4 h-4 text-neutral-600" />
      <div className={`flex items-center gap-2 ${step >= 2 ? 'text-indigo-400' : 'text-neutral-600'}`}>
        <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${step >= 2 ? 'bg-indigo-500/20' : 'bg-neutral-800'}`}>2</span> Storyboard
      </div>
      <ChevronRight className="w-4 h-4 text-neutral-600" />
      <div className={`flex items-center gap-2 ${step === 3 ? 'text-indigo-400' : 'text-neutral-600'}`}>
        <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${step === 3 ? 'bg-indigo-500/20' : 'bg-neutral-800'}`}>3</span> Render
      </div>
    </div>
  );
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-3">
                <div className="relative w-8 h-8 rounded-full overflow-hidden border-2 border-neutral-700 shadow-inner">
                  <input type="color" value={primaryColor} onChange={(e) => setPrimaryColor(e.target.value)} className="absolute -inset-2 w-12 h-12 cursor-pointer" />
                </div>
            ...
            <div className="space-y-1.5">
              ...
              {logoData ? (
                <div className="relative bg-neutral-950/50 border border-neutral-800 rounded-lg p-2 flex items-center justify-between">
                  <img src={logoData} alt="Logo" className="h-8 w-auto object-contain max-w-[150px]" />
                  <button onClick={removeLogo} className="p-1.5 text-red-400 hover:bg-red-500/10 rounded-md transition-colors"><Trash2 className="w-4 h-4" /></button>
                </div>
              ) : (
                <label className="border border-dashed border-neutral-700 bg-neutral-950/30 rounded-xl p-4 text-center cursor-pointer hover:border-indigo-500/50 hover:bg-indigo-500/5 transition-all group block">
                  ...
                  <span className="text-xs text-neutral-400 group-hover:text-neutral-300">Upload Logo (PNG preferred)</span>
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
              ...
              <div className="bg-neutral-900/50 border border-neutral-800 rounded-2xl p-6 backdrop-blur-sm">
                <div className="mb-6 pb-6 border-b border-neutral-800/60">
                  <label className="block text-xs font-semibold text-neutral-300 uppercase tracking-wider mb-3">Listing Status & Angle</label>
                  <div className="flex flex-wrap gap-2">
                    {['Coming Soon', 'Just Listed', 'For Sale', 'Open House', 'Price Reduced', 'Under Contract', 'Just Sold'].map(status => (
                      <button key={status} onClick={() => setStatusChoice(status)} className={`px-4 py-2 text-xs font-medium rounded-full transition-all ${statusChoice === status ? 'bg-indigo-500 text-white shadow-md shadow-indigo-500/20' : 'bg-neutral-950 border border-neutral-800 text-neutral-400 hover:text-neutral-200 hover:border-neutral-600'}`}>{status}</button>
                    ))}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {scenes.map((scene, index) => (
                  <div key={scene.id} className="bg-neutral-900/40 border border-neutral-800 rounded-2xl overflow-hidden hover:border-neutral-700 transition-all flex flex-col group">
                    <div className="aspect-video bg-neutral-950 flex items-center justify-center relative overflow-hidden">
                      {scene.image_url ? <img src={scene.image_url} alt={scene.room_type} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity duration-500 group-hover:scale-105" /> : <ImageIcon className="w-8 h-8 text-neutral-700" />}
                      <div className="absolute top-3 left-3 bg-black/60 backdrop-blur-md px-2.5 py-1 rounded-md text-xs font-bold text-white shadow-sm border border-white/10">{index + 1}. {scene.room_type}</div>
                    </div>
                    <div className="p-5 flex flex-col gap-4 flex-1">
                      <div><label className="block text-xs font-medium text-neutral-500 mb-1.5">Voiceover Script</label><textarea value={scene.caption} onChange={(e) => updateScene(index, 'caption', e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-lg p-3 text-sm h-20 resize-none focus:border-indigo-500 outline-none transition-all" /></div>
                      <div className="flex items-end justify-between gap-3">
                        <div className="flex-1"><label className="block text-xs font-medium text-neutral-500 mb-1.5">Camera Effect</label><select value={scene.effect} onChange={(e) => updateScene(index, 'effect', e.target.value)} className="w-full bg-neutral-950 border border-neutral-800 rounded-lg p-2.5 text-sm focus:border-indigo-500 outline-none transition-all"><option value="zoom_in">Zoom In</option><option value="drone_rise">Drone Rise</option><option value="parallax_depth">Parallax Depth</option><option value="slow_zoom_in">Slow Zoom In</option></select></div>
                        <div className="flex items-center gap-2 mb-2.5"><label className="relative inline-flex items-center cursor-pointer"><input type="checkbox" checked={scene.enable_vo} onChange={(e) => updateScene(index, 'enable_vo', e.target.checked)} className="sr-only peer" /><div className="w-9 h-5 bg-neutral-800 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-indigo-500"></div><span className="ml-2 text-xs font-medium text-neutral-400">VO</span></label></div>
                      </div>
                      <div className="flex gap-1.5 mt-auto pt-4 border-t border-neutral-800/60">
                        <button onClick={() => moveScene(index, 'up')} disabled={index === 0} className="p-2 text-neutral-500 hover:text-white hover:bg-neutral-800 rounded-lg disabled:opacity-30 transition-colors"><ArrowUp className="w-4 h-4" /></button>
                        <button onClick={() => moveScene(index, 'down')} disabled={index === scenes.length - 1} className="p-2 text-neutral-500 hover:text-white hover:bg-neutral-800 rounded-lg disabled:opacity-30 transition-colors"><ArrowDown className="w-4 h-4" /></button>
                        <button onClick={() => removeScene(index)} className="p-2 text-red-400 hover:bg-red-500/10 rounded-lg ml-auto flex items-center gap-1.5 text-xs font-medium transition-colors"><Trash2 className="w-3.5 h-3.5" /> Remove</button>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="max-w-4xl mx-auto mt-10 animate-in fade-in zoom-in-95 duration-700">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                <div className="w-full sm:w-auto flex flex-col items-center">
                  <button onClick={handleForceDownload} disabled={isDownloading} className="flex items-center justify-center gap-2 bg-white hover:bg-neutral-200 text-black font-semibold py-3.5 px-8 rounded-xl transition-all shadow-lg w-full sm:w-auto disabled:opacity-80 relative overflow-hidden">
                    {isDownloading && <div className="absolute left-0 top-0 bottom-0 bg-indigo-500/20 transition-all duration-300 ease-out" style={{ width: `${downloadProgress}%` }} />}
                    <span className="relative flex items-center gap-2 z-10">{isDownloading ? <><Loader2 className="w-5 h-5 animate-spin" /> {downloadProgress}%</> : <><Download className="w-5 h-5" /> Download MP4</>}</span>
                  </button>
                </div>
