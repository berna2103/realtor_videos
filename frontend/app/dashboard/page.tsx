...
export default function MyVideosDashboard() {
  ...
  // REFINED DELETE LOGIC (No Browser Alerts)
  const confirmDelete = async () => {
    if (!videoToDelete) return;
    const { id: videoId, url: videoUrl } = videoToDelete;

    setIsDeletingId(videoId); // Show loader on the card
    setVideoToDelete(null); // Close the modal immediately
    ...
    try {
      ...
      const { error } = await supabase.from("user_videos").delete().eq("id", videoId);
      if (error) throw error;
    } catch (error) {
      setVideos(previousVideos); // Rollback on failure
    ...
    <div className="min-h-screen bg-[#F9FAFB] text-slate-900 font-sans p-6 md:p-12 selection:bg-blue-100">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 pb-8 border-b border-slate-200">
          <div>
            <Link href="/" className="inline-flex items-center gap-2 text-sm font-bold text-blue-600 hover:text-blue-700 transition-colors mb-6 group">
            ...
            <p className="text-slate-500 mt-2 font-medium">Managing assets for <span className="text-slate-900">{userEmail}</span></p>
          </div>
          <Link href="/" className="bg-slate-900 hover:bg-black text-white font-bold py-4 px-8 rounded-2xl transition-all shadow-xl shadow-slate-200 flex items-center gap-2 active:scale-95">
        ...
        {!isLoading && videos.length > 0 && (
          <div className="bg-amber-50 border border-amber-100 rounded-2xl p-5 mb-10 flex items-start gap-4 text-amber-800 text-sm shadow-sm shadow-amber-100/50">
            ...
            <div className="flex-1">
              ...
              <p className="text-amber-700/80">Videos are hosted for 7 days. Please download your finalized MP4s for permanent access.</p>
            </div>
          </div>
        )}
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center min-h-[40vh]">
            ...
            <p className="text-slate-500 font-medium">Fetching your cinematic collection...</p>
          </div>
        ) : videos.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-[3rem] p-16 text-center max-w-2xl mx-auto mt-10 shadow-2xl shadow-slate-200/50">
            ...
            <h2 className="font-serif text-3xl font-bold text-slate-900 mb-3">Your library is empty</h2>
            <p className="text-slate-500 mb-10 text-lg leading-relaxed">Head back to the studio and turn a listing into a cinematic masterpiece.</p>
            <Link href="/" className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 px-10 rounded-2xl transition-all shadow-lg shadow-blue-100">
              Start Building
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {videos.map((video) => (
              <div key={video.id} className="bg-white border border-slate-200 rounded-[2rem] overflow-hidden hover:shadow-2xl hover:border-blue-200 transition-all duration-500 group flex flex-col relative shadow-xl shadow-slate-200/40">
                
                {/* Fixed Custom Delete Trigger */}
                <button 
                  onClick={() => setVideoToDelete({ id: video.id, url: video.video_url })}
                ...
                >
                   {isDeletingId === video.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
                </button>

                <div className="aspect-[9/16] bg-slate-900 relative overflow-hidden">
                  <video src={video.video_url} controls className="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition-opacity" preload="metadata" />
        ...
        {videoToDelete && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            <div className="bg-white border border-slate-200 rounded-[2.5rem] p-10 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-200">
              ...
              <h3 className="font-serif text-2xl font-bold text-slate-900 text-center mb-2 tracking-tight">Archive Tour?</h3>
              <p className="text-slate-500 text-center text-sm mb-8 leading-relaxed font-medium">
                This will permanently remove the cinematic tour from your library. This action cannot be undone.
              </p>
              <div className="flex flex-col gap-3">
                <button 
                  onClick={confirmDelete}
                  className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-4 rounded-2xl transition-all active:scale-95 shadow-lg shadow-red-100"
                >
                  Confirm Delete
                </button>
                <button 
